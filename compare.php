...
?>
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Header -->
					<header id="header">
						<h1>MARS<br>Top selling Cars </h1>
						<span class="avatar">


		<?php
require('config.php');
$query = "SELECT id,name,code,image,price,sale,average,yearlaunch,path from car order by YEAR(yearlaunch) DESC ,Month(yearlaunch) DESC,date(yearlaunch) DESC";
...
// $query1 = "SELECT id,name,code,image,price,sale,average,yearlaunch,path from car order by YEAR(yearlaunch) DESC ,Month(yearlaunch) DESC,date(yearlaunch) DESC LIMIT 1";
$query1 = "SELECT id,name,code,image,price,sale,average,yearlaunch,path from car ";
$response1 = mysqli_query($con,$query1);
if ($response1) {
	 while($row=mysqli_fetch_array($response1))
  {
...
$path = $row['image'];
     $name = $row['name'];
									...
									echo'<a href="'.$row['image'].'">';
										...
										echo"<h3 style='color:white'>Code:&nbsp".$row['price']."</h3>";
